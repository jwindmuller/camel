...
var CUtils = {
    ...
    parseMetadata: function(lines) {
        ...
        lines.each(function (line) {
            line = line.replace(global.metadataMarker, '');
            line = line.compact();
            if (line.has('=')) {
    ...
    getLinesFromPost: function(file) {
        var fileContent = this.getFileContent(file);
        return this.getLinesFromString(fileContent);
    },
    getLinesFromString: function(fileContent) {
        // Extract the pieces
        var lines    = fileContent.lines();
        var metadata = _.filter(lines, function (line) {
            return line.startsWith(global.metadataMarker);
        });
    ...
    replaceMetadata: function(replacements, haystack) {
        _.keys(replacements).each(function (key) {
            ...
            haystack = haystack.replace(new RegExp(global.metadataMarker + key + global.metadataMarker, 'g'), replacements[key]);
        });
    ...
    externalFilenameForFile: function(file, request) {
        ...
        retVal += file.replace('.md', '').replace(global.postsRoot, '').replace(global.postsRoot.replace('./', ''), '');
        return retVal;
    },
    ...
    rendererForFile: function(file) {
        ...
        return this.rendererForLines(lines, metadata);
    },
    rendererForLines: function(lines, metadata) {
        return {
            metadata     : metadata,
